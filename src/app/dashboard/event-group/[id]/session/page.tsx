...
export default function SessionPage({ params }: { params: { id: string } }) {
  ...
  const fetchSessions = async () => {
    try {
      ...
      const res = await axios.get(`http://localhost:3001/api/sessions`, { withCredentials: true });
      // Depending on BE, you might need to filter by event_id locally or ensure BE returns sessions for this group
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Kelola jadwal sesi (rundown) dalam acara ini.</p>
        </div>
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200">
          <table className="table-base w-full">
            <thead className="table-header bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-6 py-4 font-semibold text-gray-600">Nama Sesi</th>
                <th className="px-6 py-4 font-semibold text-gray-600">Waktu</th>
                <th className="px-6 py-4 font-semibold text-gray-600">Lokasi</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                sessions.map((session) => (
                  <tr key={session.id} className="table-row hover:bg-gray-50/50 transition-colors">
