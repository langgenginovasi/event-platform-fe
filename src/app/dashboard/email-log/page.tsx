...
export default function EmailLogPage() {
  ...
  const { data, isLoading, mutate } = useSWR<any>(
    `/email-batches?limit=50${typeFilter ? `&type=${typeFilter}` : ""}`,
    fetcher,
    ...
    <div className="flex flex-col space-y-6 pb-20 md:pb-0">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <p className="text-sm text-gray-500">
          Log pengiriman email secara global (test, bulk, dan single). Diperbarui otomatis setiap 5 detik.
        </p>
        ...
        <ContentCard>
          <ContentCardBody>
            <div className="flex flex-col items-center justify-center py-16 text-gray-400 space-y-3">
              ...
              <p className="text-sm font-medium">Belum ada riwayat pengiriman email.</p>
              <p className="text-xs text-center max-w-xs">
                Log akan muncul di sini setelah Anda melakukan pengiriman email (test, bulk, atau single).
              </p>
            </div>
          </ContentCardBody>
        </ContentCard>
